...
class BowlConfig(Config):
    ...
    STEPS_PER_EPOCH = 332//IMAGES_PER_GPU
    VALIDATION_STEPS = 40//IMAGES_PER_GPU ## We are training with the whole dataset so validation is not very meaningfull, I put a two here so it is faster. We either use train loss or calculate in a separate procceses the mAP for each epoch
