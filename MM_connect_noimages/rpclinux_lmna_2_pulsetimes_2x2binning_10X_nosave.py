...
class LinuxService(rpyc.Service):
    class exposed_ImageAnalysis(object):
        ...
        def exposed_run_pipeline_on_image(self, image_name, img):
            ...
            #normalize and save
            img_norm = self.normalize_image(img[:,:,1], percentile=99.5) #only dendra channel (1), not LMNA channel (0)
        ...
        def analyze_image(self, image_name, norm_dendra_image, LMNA_image, image_dir):
            ...
            with warnings.catch_warnings():
                ...
                for i in range(n_cells):
                    ...
                    crop = self.normalize_image(crop, percentile=90)
                ...
                for i in range(n_cells):
                    ...
                    if (predictions[i] == 2) & (cell_size[i] >= cell_size_cutoff) & (boundary_cell[i] == 0):
                ...
                print('Activated ' + str(n_activated_cells) + ' cells!')
...
if __name__ == "__main__":
    ...
    output_dir = "./u2os-lmna/8-5-19_output_test"
    model_file = "deepretina_final.h5"
    model_path = os.path.join(root_dir, model_file)
    classification_model_path = '/home/srirampendyala/Projects/U2OS-LMNA/classifier-images/imageset_divided/4x4_binning_10X' #use sriram's resnet34 model trained on 4x4 binned 20X crops with multiple classes
    save_every = 500
    cell_size_cutoff = 0
    batch_size = 2048
    expand_pixels = 1
