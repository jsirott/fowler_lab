    ...
    class exposed_ImageAnalysis(object):
        ...
        def analyze_image(self, image_name, norm_dendra_image, LMNA_image, image_dir):
            ...
            with warnings.catch_warnings():
                ...
                for i in range(n_cells):
                    if (predictions[i] == 0) & (cell_size[i] >= cell_size_cutoff) & (boundary_cell[i] == 0):
                        output_mask[crop_y_min[i]:crop_y_max[i],crop_x_min[i]:crop_x_max[i]] = np.logical_or(output_mask[crop_y_min[i]:crop_y_max[i],crop_x_min[i]:crop_x_max[i]], results[0]['masks'][crop_y_min[i]:crop_y_max[i],crop_x_min[i]:crop_x_max[i],i])
                    global image_number
                    global celldata
            ...
            return output_mask[padding[0][0]:image_metas[4]-padding[0][1],padding[1][0]:image_metas[5]-padding[1][1]]

if __name__ == "__main__":

    #Set global variables
    root_dir = "/home/srirampendyala/Projects/DSB_2018-master/"
    output_dir = "./u2os-lmna/5-29-19_output"
    model_file = "deepretina_final.h5"
    model_path = os.path.join(root_dir, model_file)
    classification_model_path = '/home/srirampendyala/Projects/U2OS-LMNA/crops/jpegs_titration/' #use HJK's resnet34 model
    save_every = 1000
    cell_size_cutoff = 500
    batch_size = 512
    expand_pixels = 2
    boundary_size = 5
    image_height = 1536
