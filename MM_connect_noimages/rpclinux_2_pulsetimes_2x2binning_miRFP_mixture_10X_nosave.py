...
class LinuxService(rpyc.Service):
    class exposed_ImageAnalysis(object):
        ...
        def exposed_finalize(self):
            ...
            cell_dataframe = pd.DataFrame.from_dict(celldata,orient='index',columns=['ImageNumber', 'ImageDir', 'ObjectNumber', 'CellNumber', 'Size_X', 'Size_Y', 'Centroid_X', 'Centroid_Y', 'Cell_Size', 'Boundary_Cell', 'Mean_LMNA_Intensity', 'Mean_Background_Intensity', 'Mean_Divided_Intensity', 'Activated'])
        ...
        def exposed_run_pipeline_on_image(self, image_name, img):
            ...
            #normalize and save
            img_norm = self.normalize_image(img[:,:,1], percentile=97) #only dendra channel (1), not LMNA channel (0)

            #analyze
            output_mask = self.analyze_image(image_name, img_norm, img[:,:,0], image_dir)
        ...
        def normalize_image(self, img, percentile=99.9):
        ...
        def analyze_image(self, image_name, norm_dendra_image, LMNA_image, image_dir):
            ...
            with warnings.catch_warnings():
                ...
                LMNA_image = np.pad(LMNA_image, padding[0:2], mode='constant')
                ...
                mean_LMNA_intensity = np.zeros(n_cells)
                mean_background_intensity = np.zeros(n_cells)
                mean_divided_intensity = np.zeros(n_cells)
                ...
                for i in range(n_cells):
                    ...
                    # Compute LMNA intensity mean
                    mean_LMNA_intensity[i] = np.true_divide(crop.sum(),(crop!=0).sum())
                    mean_background_intensity[i] = np.true_divide(crop_background.sum(),(crop_background!=0).sum())
                    mean_divided_intensity[i] = mean_LMNA_intensity[i] / mean_background_intensity[i]
                ...
                if n_cells > 0:
                    ...
                    predictions = mean_divided_intensity >= mean_divided_intensity_cutoff
                else:
                ...
                for i in range(n_cells):
                    if ((predictions[i] == 0) | (predictions[i] == 1)) & (cell_size[i] >= cell_size_cutoff) & (boundary_cell[i] == 0):
                    ...
                    if (predictions[i] == 1) & (cell_size[i] >= cell_size_cutoff) & (boundary_cell[i] == 0):
                    ...
                    celldata[object_number] = [image_number, image_dir, object_number, i, cell_x_dim[i], cell_y_dim[i], cell_x_centroid[i], cell_y_centroid[i], cell_size[i], boundary_cell[i], mean_LMNA_intensity[i], mean_background_intensity[i], mean_divided_intensity[i], predictions[i]]
                    object_number += 1
...
if __name__ == "__main__":
    ...
    output_dir = "./test/test_10X_08_12_19"
    model_file = "deepretina_final.h5"
    model_path = os.path.join(root_dir, model_file)
    #classification_model_path = '/home/srirampendyala/Projects/U2OS-LMNA/classifier-images/imageset_divided/2x2_binning' #use hyeon-jin's resnet34 model trained on 2x2 binned crops with multiple classes
    save_every = 500
    cell_size_cutoff = 0
    batch_size = 2048
    expand_pixels = 1
    ...
    image_width = 870
    crop_size = 32
    mean_divided_intensity_cutoff = 1.05
    celldata = {}
    image_number = 0
    imagedata = {}
    object_number = 0
    ...
    inference_config.DETECTION_MAX_INSTANCES = 2048
    inference_config.POST_NMS_ROIS_INFERENCE = 4000
    inference_config.RPN_NMS_THRESHOLD = 0.35
    inference_config.DETECTION_MIN_CONFIDENCE = 0.35
