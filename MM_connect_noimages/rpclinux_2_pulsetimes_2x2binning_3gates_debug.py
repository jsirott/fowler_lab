...
class LinuxService(rpyc.Service):
    class exposed_ImageAnalysis(object):
        ...
        def exposed_run_pipeline_on_image(self, image_name, img):
            ...
            #import image
            img = np.frombuffer(base64.decodebytes(img), np.uint8).reshape((image_height,image_width))
            #np.save(os.path.join(image_dir,'image.npy'), img)

            #normalize and save
            img_norm = self.normalize_image(img) #only 1 channel image
        ...
        def analyze_image(self, image_name, norm_dendra_image, image_dir):
            ...
            with warnings.catch_warnings():
                ...
                if n_cells > 0:
                    ...
                    predictions = np.random.randint(4, size=n_cells)
                else:
                ...
                output_mask3 = np.zeros(image_metas[4:6], dtype='bool')
                for i in range(n_cells):
                    if ((predictions[i] == 1) | (predictions[i] == 2) | (predictions[i] == 3)) & (cell_size[i] >= cell_size_cutoff) & (boundary_cell[i] == 0):
                    ...
                    if ((predictions[i] == 2) | (predictions[i] == 3)) & (cell_size[i] >= cell_size_cutoff) & (boundary_cell[i] == 0):
                    ...
                    if (predictions[i] == 3) & (cell_size[i] >= cell_size_cutoff) & (boundary_cell[i] == 0):
                        output_mask3[crop_y_min[i]:crop_y_max[i],crop_x_min[i]:crop_x_max[i]] = np.logical_or(output_mask2[crop_y_min[i]:crop_y_max[i],crop_x_min[i]:crop_x_max[i]], results[0]['masks'][crop_y_min[i]:crop_y_max[i],crop_x_min[i]:crop_x_max[i],i])
                    global image_number
                    global celldata
                    ...
                    object_number += 1
                output_mask = (output_mask1[padding[0][0]:image_metas[4]-padding[0][1],padding[1][0]:image_metas[5]-padding[1][1]].astype('uint8') + output_mask2[padding[0][0]:image_metas[4]-padding[0][1],padding[1][0]:image_metas[5]-padding[1][1]].astype('uint8') + output_mask3[padding[0][0]:image_metas[4]-padding[0][1],padding[1][0]:image_metas[5]-padding[1][1]].astype('uint8'))                
...
if __name__ == "__main__":
    ...
    output_dir = "./test/test_12_1_19_threegates"
