...
def Docommand(param):
    ...
    np_pixels_phased = np.stack([np_pixels[:,np.arange(1,width,3)], np_pixels[:,np.arange(2,width,3)]], axis=-1)
    ...
    np_output_mask = np.kron(np_output_mask, np.ones((2,2))).astype(np.uint8)

    if writeTimerMessages:
         mm.PrintMsg("Numpy array new size: " + str(np_output_mask.dtype))
    ...
    output_mask = Array.CreateInstance(System.Byte, 2*width, 2*height)
    ...
    _,_,_,_,_,newImageHandle = mm.CreateImage(2*width, 2*height, 8, newImageName, newImageHandle)

    # Write image
    start = time.time()
    _ = mm.WriteImage(newImageHandle, 0, 0, 2*width, 2*height, 8, 0, 0, output_mask)
    end = time.time()
    end0 = time.time()
